        ...
        try:
            ...
            if 'pfbid' in ray['id']: pass
                ...
                if ray in self.result: pass
                    ...
                    open('dump-grup.txt','a+').write('%s|%s\n'%(ray['id'],ray['name']))
                    self.dump += 1
        except Exception as e: pass
                ...
                try:
                    ...
                    if 'pfbid' in ray['id']: pass
                        ...
                        if ray in self.result: pass
                            ...
                            self.result.append(ray)
                            print('%s|%s'%(ray['id'],ray['name']))
                            open('dump-komen.txt','a+').write('%s|%s\n'%(ray['id'],ray['name']))
                            self.dump += 1
                except Exception as e: pass
        except Exception as e: pass
                    ...
                    try:
                        ...
                        if 'pfbid' in ray['id']: pass
                            ...
                            if ray in self.result: pass
                                ...
                                self.result.append(ray)
                                print('%s|%s'%(ray['id'],ray['name']))
                                open('dump-react.txt','a+').write('%s|%s\n'%(ray['id'],ray['name']))
                                self.dump += 1
                    except Exception as e: pass
            except Exception as e: pass
        ...
        try:
            ...
            if nama=='Content not found' or nama=='Log in to Facebook': pass
                ...
                if ray in self.result: pass
                    ...
                    self.result.append(ray)
                    print('%s|%s'%(ray['id'],ray['name']))
                    open('dump-search.txt','a+').write('%s|%s\n'%(ray['id'],ray['name']))
                    self.dump += 1
        except Exception as e: pass
        ...
        try:
            ...
            if 'next=' in str(req.url): pass
                ...
                if nama=='Content not found' or nama=='Error Facebook' or re.search(r'\d',nama): pass
                    ...
                    print('%s|%s'%(id,nama))
                    open('dump-random.txt','a+').write('%s|%s\n'%(id,nama))
        except Exception as e: pass
...
if __name__ == '__main__':
    mod()
    print("=======[Menu]========")
    print("1. Dump ID Grup/Page")
    print("2. Dump ID Search")
    print("3. Dump ID Random")
    i = input("Pilih : ")
    if i in ['1']:
        TimelineNoLogin()
    elif i in ['2']:
...
# Rikode script for prem mwhehehehehe
