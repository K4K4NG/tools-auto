...
class Main(object):
    ...
    def cek(self):
        try:
            r = par(
                ses.get(
                    "https://mbasic.facebook.com/security/2fac/setup/intro/metadata/?source=1",
                    cookies=self.coki).text, 'html.parser')
        ...
        try:
            lanjut = r.find("a",
                            string="Gunakan Aplikasi Autentikasi").get("href")
        except:
        ...
        try:
            ...
            if "Demi keamanan, masukkan ulang kata sandi Anda untuk melanjutkan." in __r:
                print(
                    " * Demi keamanan, masukkan kata sandi Facebook anda untuk melanjutkan."
                )
                ...
                memek.update({"pass": sandi})
                response = ses.post(lanjut.get("action"),
                                    cookies=self.coki,
                                    data=memek).text
    ...
    def spam(self, cokii):
        print(
            " ! Untuk menghindari checkpoint tools akan mengspam komen di postingan Author sebanyak 14 spam.\n * Proses sedang berjalan harap tunggu...\n"
        )
        sp.Main(cokii).gasken(14, "Latif Ganteng:v")
